"""
YOLOv8 MULTI-CLASS SINGLE IMAGE GRAD-CAM
========================================
Overlays all detected fine-grain features onto a single image 
using a unique color for each class, complete with a color legend.
"""
...
# Define distinct RGB colors for each class for the final plot
CLASS_COLORS = {
    'defect':  (255, 0, 0),     # Red
    'brown':   (255, 165, 0),   # Orange / Brownish trend
...
def generate_single_image_gradcam():
    ...
    for crop_name in TARGET_CROPS:
        ...
        # Base canvas for compounding custom colored activations
        ...
        if results.masks is not None:
            ...
            for i, cls_idx in enumerate(classes):
                if cls_idx < len(FG_CLASSES):
                    ...
                    # Continuous activation map weighted by confidence
                    ...
                    # Smooth the mask into a Grad-CAM low-res glow
                    ksize = int(max(H, W) * 0.1) | 1
                    smoothed_map = cv2.GaussianBlur(prob_map, (ksize, ksize), 0)
                    
                    # Normalize smoothed map locally to retain intensity structure
                    if smoothed_map.max() > 0:
                        smoothed_map = smoothed_map / smoothed_map.max()

                    # Convert single channel intensity to designated class RGB color
                    ...
                    # Overlay onto the unified canvas using element-wise maximum to avoid over-saturation blending
                    combined_heatmap = np.maximum(combined_heatmap, class_color_heatmap)

        # Final blend: Overlay the compounded multi-class heatmaps onto original image
        alpha = 0.45
        combined_heatmap = np.uint8(combined_heatmap)
        
        # Apply mask thresholding so color only applies where a features exists
        ...
        # Panel 2: Multi-Class Compound Grad-CAM
        axes[1].imshow(final_overlay)
        axes[1].set_title("Unified Fine-Grain XAI Map", fontsize=12, fontweight='bold', color='darkred')
        axes[1].axis("off")

        # Create interactive legend bounding boxes for detected features
        legend_patches = []
        for cls_name in detected_in_image:
            # Convert 0-255 RGB tuple to 0-1 range for matplotlib patches
