...
df_fg    = df_train[df_train[FG_COLS].sum(axis=1) > 0].copy().reset_index(drop=True)
...
def get_sample_weights(y_arr):
    ...
    return np.array([cw[yi] for yi in y_arr])
...
# Train on all train+val, predict on test
X_tr = np.hstack([clip_all_n, fg_all_n, pseudo_all_n])
X_te = np.hstack([clip_te_n,  fg_te_n,  bert_te_n])

sc_fin = StandardScaler()
X_tr_s = sc_fin.fit_transform(X_tr)
X_te_s = sc_fin.transform(X_te)
...
final_model.fit(X_tr_s, y_tr_enc, sample_weight=sw)
final_model.save_model(str(RESULTS_DIR/'final_fusion_model.json'))

y_pred_final = final_model.predict(X_te_s)

print("\nFINAL TEST RESULTS — Full Fusion:")
...
# Image-only baseline on test
img_model = xgb.XGBClassifier(
    objective='multi:softprob', num_class=4,
    max_depth=5, n_estimators=200, random_state=42, verbosity=0)
img_model.fit(sc_fin.fit_transform(clip_all_n), y_tr_enc, sample_weight=sw)
y_pred_img = img_model.predict(sc_fin.transform(clip_te_n))

# McNemar's test
from statsmodels.stats.contingency_tables import mcnemar
cf = (y_pred_final == y_te_enc)
ci = (y_pred_img   == y_te_enc)
tbl = np.array([[(cf&ci).sum(),(~cf&ci).sum()],
                [(cf&~ci).sum(),(~cf&~ci).sum()]])
mcn = mcnemar(tbl, exact=False, correction=True)
print(f"\nMcNemar (fusion vs image-only):")
print(f"  statistic={mcn.statistic:.4f}  p={mcn.pvalue:.6f}")
print(f"  {'SIGNIFICANT' if mcn.pvalue<0.05 else 'not significant'} at p<0.05")
...
fig, axes = plt.subplots(1, 2, figsize=(13, 5.5))
fig.patch.set_facecolor(PAL['bg'])
fig.suptitle('Final Test Set Results (held-out)',
             fontsize=13, fontweight='bold', color=PAL['text'])

img_cm  = confusion_matrix(y_te_enc, y_pred_img)
fus_cm  = confusion_matrix(y_te_enc, y_pred_final)

for ax, cm, title in zip(
        axes,
        [img_cm, fus_cm],
        ['Image only (test)',
         f'Full Fusion (test)\nKappa={kappa_test:.3f}  p={mcn.pvalue:.4f}']):
    pct = cm.astype(float)/cm.sum(axis=1, keepdims=True)*100
    sns.heatmap(pct, annot=True, fmt='.0f', cmap='Blues',
...
print(f"\n  TEST SET (held-out 623 crops, run once):")
print(f"  Full fusion: wF1={test_report['weighted avg']['f1-score']:.3f}  "
      f"Kappa={kappa_test:.3f}  McNemar p={mcn.pvalue:.4f}")
print(f"\n  ★ = main contribution")
